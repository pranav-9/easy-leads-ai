...
export class WhatsappController {
  ...
  @Post('send-template')
  async sendTemplate(
    @Body() body: { phone: string; templateName: string; components: any[] },
  ) {
  ...
  async startConversation(@Param('leadId') leadId: string) {
    ...
    try {
      ...
      this.logger.error(
        `Error starting conversation with lead ${leadId}:`,
        error.stack,
      );
  ...
  async webhook(@Body() body: any) {
    this.logger.debug(
      'Received webhook from WhatsApp - controller entry point',
    );
    ...
    try {
      ...
      this.logger.debug(
        'Forwarding webhook data to WhatsappService.processWebhook',
      );
      const result = await this.whatsappService.processWebhook(body);

      if (result) {
        this.logger.debug(
          `Webhook processing result: ${JSON.stringify(result)}`,
        );
      } else {
        this.logger.debug(
          'Webhook processed with no result (null/undefined returned)',
        );
      }
