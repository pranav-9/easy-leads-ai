...
export class BotpressService {
    ...
    private async sendToBotpress(phone: string, message: string): Promise<any> {
        try {
            ...
            const response = await this.httpService.post(botpressUrl, payload, {
                headers: {
            ...
            if (error.response?.status === 401) {
                throw new Error('Authentication failed with Botpress API. Check your API token.');
            } else if (error.response?.status === 404) {
                throw new Error('Botpress API endpoint not found. Check the API URL.');
            } else {
                throw new Error(`Failed to send message via Botpress: ${error.message}`);
            }
        }
    }
